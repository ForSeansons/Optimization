...
| `limegreen` | 实线 | Huber Gradient | 线性渐近，受平滑误差限制在 $10^{-2}$ 左右。 |
...
| `orange` | 实线 | ADMM ($\rho=0.5$) | 步长偏大导致后期拖尾。 |
| `magenta` | 实线 | ADMM ($\rho=1$) | 低维表现最佳，速度与稳定性兼顾。 |
| `cyan` | 实线 | ADMM ($\rho=2$) | 高维下新的最佳点，抑制震荡。 |
| `purple` | 实线 | ADMM ($\rho=5$) | 惩罚过大，更新幅度受限。 |
...
| ADMM ($\rho=0.5$) | 52.18 | 52.00 | 46 | 61 | 100% |
| ADMM ($\rho=1$) | 32.89 | 33.00 | 29 | 36 | 100% |
| ADMM ($\rho=2$) | 43.75 | 43.00 | 36 | 53 | 100% |
| ADMM ($\rho=5$) | 86.70 | 86.00 | 69 | 100 | 93.0% |
...
| ADMM ($\rho=0.5$) | 250.00 | 250.00 | 250 | 250 | 0% |
| ADMM ($\rho=1$) | 138.30 | 138.50 | 132 | 146 | 100% |
| ADMM ($\rho=2$) | 80.00 | 79.50 | 77 | 84 | 100% |
| ADMM ($\rho=5$) | 106.00 | 105.50 | 92 | 116 | 100% |
...
| FISTA / ISTA / Stochastic Proximal | $O(np)$ 或 $O(nb)$ | $X^T(X\beta - y)$（或 mini-batch 版本） | 线性增长，动量/重启决定震荡水平。 |
| Huber Gradient 家族 | $O(np)$ | 平滑梯度计算 + 步长限制 | 线性但存在误差地板。 |
| ADMM | 预处理 $O(p^3)$，迭代 $O(p^2)$ | 求解 $(X^TX + \rho I)^{-1}$ | **立方**：高维下成为主要瓶颈。 |
